...
class API {
    ...
    async auth(username, password) {
        ...
        const random = Math.floor( Math.random() * (max + 1 - 0) ) + 0 ;
        const proxy = process.env.SOCKS + proxies.all[random];
        ...
        this.asidcookie = response.headers["set-cookie"].find((elem) => /^asid/.test(elem)); //asidcookie
        ...
        if (access_tokens.data.type == "multifactor") {
          this.asidcookie = access_tokens.headers["set-cookie"].find((elem) => /^asid/.test(elem)); //asidcookie
          throw new Error("2fa");
        }
    ...
    async authWithCode(asidcookie, code) {
      ...
      // update access token
      var tokens = parseTokensFromUrl(response.data.response.parameters.uri);
      this.access_token = tokens.access_token;
